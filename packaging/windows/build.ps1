...
# Optional: bake ANTHROPIC_API_KEY / GEMINI_API_KEY / ELEVENLABS_API_KEY in as defaults
# so end users don't need to configure anything (see
# .github/workflows/build-windows-installer.yml, which sources these from GitHub
# Actions secrets — never from a checked-in file). Read by config.py's load_settings()
# as the lowest-priority source; the settings screen still overrides it. Silently
# produces no file (i.e. no defaults) if the env vars are unset, e.g. for a local
# build.ps1 run without them exported.
$bundledDefaults = @{}
if ($env:ANTHROPIC_API_KEY)  { $bundledDefaults["ANTHROPIC_API_KEY"] = $env:ANTHROPIC_API_KEY }
