...
Kirinuki is a tool that automatically generates clip videos from YouTube live streams. It downloads a video, transcribes audio with Groq Whisper, uses Codex CLI to suggest interesting segments, and renders MP4 clips with subtitles via Remotion (React-based video renderer).
...
Required system tools: `yt-dlp`, `ffmpeg`, `ffprobe`, `Codex` (CLI), `npx`
...
- `web/pipeline.py` — All heavy lifting: `download_video` (yt-dlp), `run_transcription` (delegates to `audio_chunking_code.py`), `suggest_clips_from_result` (calls `Codex -p ... --output-format text` as subprocess), `render_clip` (calls `npx remotion render`).
...
URL → yt-dlp → .mp4 (downloads/)
      ...
      Groq Whisper → *_full.json (transcriptions/)
            ↓
      Codex CLI → clips_*.json (transcriptions/)
            ↓
...
- The `Codex` CLI must be authenticated and available in PATH — `suggest_clips_from_result()` calls it directly as a subprocess.
