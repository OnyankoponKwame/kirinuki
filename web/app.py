...
class ThemeReq(BaseModel):
    ...
    titleMaxLines: int | None = None
    captionFontSize: int | None = None
    splitTopRatio: float | None = None
    captionPositionY: float | None = None
...
def split_geometry(req: SplitGeometryReq):
    ...
    The title bar's height/margin/line-count and the split-mode top/bottom ratio are
    all per-theme (titleBarMinHeight / titleTopMargin / titleMaxLines / splitTopRatio),
    so `theme` selects which one to resolve them from — same key the clip cards send
    as `theme` when building render props.
    """
    import premiere_export as px
    ...
    title_max_lines = theme_colors.get("titleMaxLines")
    title_max_lines = 2 if title_max_lines is None else int(title_max_lines)
    split_top_ratio = theme_colors.get("splitTopRatio")
    split_top_ratio = 4.5 if split_top_ratio is None else float(split_top_ratio)
    g = px.split_geometry(
        req.title, max(1, min(9, split_top_ratio)),
...
# studio-src/ 側にだけ生成するファイル（src/ にはスタブを置く、もしくは存在しない）
_STUDIO_GENERATED = ("studioData.json", "StudioRoot.tsx", "studio-entry.ts")

_STUB_COMPOSITIONS_TSX = """\
// Auto-generated by Kirinuki web app — do not edit manually
// 本物のプレビュー用コンポジションは studio-src/StudioRoot.tsx に生成される
// （_sync_studio_src / _generate_studio_root 参照）。CLI レンダリングが使うのは
// Root.tsx の ClipComposition だけなので、こちらは import を満たすだけの空実装でよい。
export const StudioCompositions: React.FC = () => null;
"""

_STUB_STUDIO_DATA = '{\n  "videoSrc": "",\n  "segments": [],\n  "clips": []\n}\n'

# studio-src/ のエントリポイント。ファイル名が「-entry」で終わることで、Remotion Studio の
# プロジェクトルート探索が同じディレクトリの StudioRoot.tsx を「ルートファイル」として直接
# 見つける（Cannot save default props のトラブルシューティング参照）。この命名規則を使わない
# 場合、ルートファイル探索は常に remotion/<remotionRoot>/src/Root.tsx （実ソース側）を拾って
# しまい、そちらには各クリップの <Composition id="clip-..."> が存在しないため
# 「Could not find defaultProps for composition」で失敗する — remotionRoot は Studio の
# 起動時のエントリポイントに関わらず cwd（= REMOTION_DIR）直下の package.json から固定で
# 決まるため、studio-src/index.ts のような通常の名前では回避できない。
_STUDIO_ENTRY_FILENAME = "studio-entry.ts"
_STUDIO_ROOT_FILENAME = "StudioRoot.tsx"
...
def _generate_studio_root(
    ...
    """Generate StudioRoot.tsx — the file registered by studio-src/studio-entry.ts — with
    defaultProps inlined as object literals directly in the JSX (not through a named const).

    Two separate Remotion requirements are at play here, both documented at
    https://www.remotion.dev/docs/troubleshooting/cannot-save-default-props:
    - defaultProps must be an inline object literal for the Props editor to save edits back.
    - the literal <Composition> elements must live in whatever file Remotion resolves as
      "the root file" — it parses only that one file's AST, it does not follow imports. That
      is why these are generated directly here rather than in a component (e.g. the old
      StudioCompositions) that a Root.tsx would merely render; a JSX reference doesn't count
      as "found in the root file" for this purpose. See _STUDIO_ENTRY_TS for how the entry
      filename makes Remotion resolve *this* file, in studio-src/, as that root file.
...
async def open_studio(req: StudioReq):
    ...
    # studio-src/ 側のエントリポイント（内容は固定なので変化があるときだけ書く）
    _write_if_changed(STUDIO_SRC_DIR / _STUDIO_ENTRY_FILENAME, _STUDIO_ENTRY_TS)
    ...
    tsx = _generate_studio_root(
        video_path.name, segments, merged_clips, studio_src_aspect, uuid.uuid4().hex
    )
    (STUDIO_SRC_DIR / _STUDIO_ROOT_FILENAME).write_text(tsx, encoding="utf-8")
        ...
        with open(studio_log, "w", encoding="utf-8") as logf:
            _studio_proc = subprocess.Popen(
                cfg.get_npx_cmd() + [
                    ...
                    # remotion/src を守るため（_sync_studio_src 参照）。ファイル名を
                    # 「-entry」で終わらせているのは defaultProps 保存のルートファイル
                    # 探索を studio-src/StudioRoot.tsx に向けるため（_STUDIO_ENTRY_TS 参照）
                    f"studio-src/{_STUDIO_ENTRY_FILENAME}",
                    "--no-open",
