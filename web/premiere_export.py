...
■ 取り込み手順（Adobe Premiere Pro）
  1. mp4 を Premiere のプロジェクトパネルにドラッグします。
     そのままタイムラインパネルにドラッグすると、素材に合ったサイズ
     （1080×1920 など）のシーケンスが自動で作られます。
  2. 字幕は `*.srt` を File > Import... で読み込み、
     シーケンスのキャプショントラックにドラッグしてください。
     mp4 とタイムコードが一致しているので、そのまま合います。
  3. 字幕の文字やスタイルは Premiere 側で自由に編集できます。

  ※ DaVinci Resolve / Final Cut Pro など他の編集ソフトでも、
     mp4 と srt をそのまま読み込めば同じように使えます。
